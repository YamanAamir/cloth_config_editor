...
const SweatPants = ({ data, onUpdate, isAppReady, logos }) => {
  ...
  const pressureOptions = data?.pressureOptions || {
    rightLegText: "",
    rightLegFlag: "",
    rightLegLogoPredefined: "",
    rightLegLogoCustom: "",
    rightLegType: "", // 'flag' | 'logo' | ''

    leftLegText: "",
    leftLegFlag: "",
    leftLegLogoPredefined: "",
    leftLegLogoCustom: "",
    leftLegType: "",
  ...
  useEffect(() => {
    // const colorMap = {
    //   red: 'SweatPant:red',
    //   orange: 'SweatPant:orange',
    //   lime: 'SweatPant:lime',
    //   kit: 'SweatPant:kit',
    //   'light blue': 'SweatPant:light blue',
    //   turquoise: 'SweatPant:turquoise',
    //   navy: 'SweatPant:navy',
    //   black: 'SweatPant:black',
    //   'white (black print)': 'SweatPant:white',
    // };
  ...
  useEffect(() => {
    ...
    const message = `SweatPant:size:${selectedSize}`;
    ["preview-iframe", "preview-iframe2"].forEach((id) => {
      const iframe = document.getElementById(id);
      if (iframe?.contentWindow) {
        iframe.contentWindow.postMessage(message, "*");
  ...
  useEffect(() => {
    ...
    areas.forEach((area) => {
      ...
      prevPressureOptionsRef.current[area] = {
        text,
        flag,
        logoPre,
        logoCustom,
      ...
      ["preview-iframe", "preview-iframe2"].forEach((id) => {
        const iframe = document.getElementById(id);
        if (iframe?.contentWindow) {
          const msg = `SweatPant:${area}_opacity: ${opacity}`;
          iframe.contentWindow.postMessage(msg, "*");
        }
      });

      // Diffuse
      getDiffuseBase64(hasFlag ? flag : "", text, (diffuseBase) => {
            ...
            if (hasLogo) {
              ...
              logoImg.onload = () => {
                ...
                ["preview-iframe", "preview-iframe2"].forEach((id) => {
                  const iframe = document.getElementById(id);
                  if (iframe?.contentWindow) {
                    const msg = `SweatPant:${area}_diffuse: ${finalDiffuse}`;
                    iframe.contentWindow.postMessage(msg, "*");
                  }
                });
              };
              ...
              ["preview-iframe", "preview-iframe2"].forEach((id) => {
                const iframe = document.getElementById(id);
                if (iframe?.contentWindow) {
                  const msg = `SweatPant:${area}_diffuse: ${diffuseBase}`;
                  iframe.contentWindow.postMessage(msg, "*");
                }
              });
            }
