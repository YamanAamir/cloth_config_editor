...
const SweatShirt = ({ data, onUpdate, isAppReady, logos }) => {
  ...
  const CANVAS_WIDTH = 320, TEXT_HEIGHT = 120, FLAG_HEIGHT = 240;
  ...
  const getDiffuseBase64 = (flag, logoPre, logoCustom, text, callback, flag2 = "", flagCount = 1) => {
    ...
    if (flag && flagImages[flag]) {
      if (flag2 && flagImages[flag2]) {
        Promise.all([loadImage(flagImages[flag]), loadImage(flagImages[flag2])])
          .then(([img1, img2]) => {
            const gap = 10, flagH = FLAG_HEIGHT / 2, flagW = (CANVAS_WIDTH - gap) / 2, startX = (CANVAS_WIDTH - flagW) / 2;
    ...
    if (logoSrc) {
      loadImage(logoSrc).then(img => {
        ...
        const w = img.width * ratio * 0.9, h = img.height * ratio * 0.9;
  ...
  useEffect(() => {
    if (logos && logos.length === 1) {
      const fields = ["rightChestLogoPredefined", "leftChestLogoPredefined", "rightSleeveLogoPredefined", "leftSleeveLogoPredefined"];
  ...
  useEffect(() => {
    const colorMap = {
      red: "SweatShirt:red", black: "SweatShirt:black", white: "SweatShirt:white", natural: "SweatShirt:natural",
      "heather grey": "SweatShirt:heatherGrey", navy: "SweatShirt:navy", "light pink": "SweatShirt:lightPink",
      "olive green": "SweatShirt:oliveGreen", blue: "SweatShirt:blue", purple: "SweatShirt:purple",
    };
  ...
  useEffect(() => {
    ...
    const msg = `SweatShirt:size:${selectedSize}`;
    ["preview-iframe", "preview-iframe2"].forEach(id => { const f = document.getElementById(id); if (f?.contentWindow) f.contentWindow.postMessage(msg, "*"); });
  }, [selectedSize, isAppReady]);

  const prevRef = React.useRef({});
  useEffect(() => {
    ["rightChest", "leftChest", "rightSleeve", "leftSleeve"].forEach(area => {
      ...
      ["preview-iframe", "preview-iframe2"].forEach(id => { const f = document.getElementById(id); if (f?.contentWindow) f.contentWindow.postMessage(`SweatShirt:${area}_opacity: ${opacity}`, "*"); });
      getDiffuseBase64(flag, logoPre, logoCustom, text, diffuse => {
        ["preview-iframe", "preview-iframe2"].forEach(id => { const f = document.getElementById(id); if (f?.contentWindow) f.contentWindow.postMessage(`SweatShirt:${area}_diffuse: ${diffuse}`, "*"); });
      }, flag2, flagCount);
    });
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl max-h-[85vh] flex flex-col overflow-hidden border border-slate-200">
        <div className="flex items-center justify-between px-8 py-7 border-b border-slate-50 bg-white/50 sticky top-0 z-10">
          ...
          <button onClick={() => setShowFlagModal(false)} className="p-3 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-2xl transition-all group">
          ...
          {currentField.includes("Logo") ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
              {logos && logos.map(logo => (
                <button key={logo.id} onClick={() => selectLogo(logo.name, logo.id)}
                  className="group flex flex-col items-center p-2 rounded-3xl hover:bg-white hover:shadow-xl transition-all duration-300"
                >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {countries.map(country => (
                ...
                >
                  <div className="w-16 h-12 rounded-lg overflow-hidden shadow-sm bg-slate-100">
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-gray-50">
            ...
            <div className="grid grid-cols-4 gap-4">
              {colors.map(color => (
                <div key={color.name} className="flex flex-col items-center">
                  <button onClick={() => onUpdate({ selectedColor: color.name })} className="relative w-12 h-12 rounded-lg transition-all focus:outline-none"
      ...
      <div className={activeTab === "pressure" ? "mt-10" : ""} style={activeTab !== "pressure" ? { visibility: "hidden", position: "absolute", pointerEvents: "none", height: 0, overflow: "hidden" } : {}}>
        <Test postEx="SweatShirt:" pressureOptions={pressureOptions} isAppReady={isAppReady}
          onUpdate={update => {
