...
const Shorts = ({ data, onUpdate, isAppReady, logos, maxCharsText = 25, activeTab: externalTab }) => {
  ...
  const getEffectiveDimensions = (flagCount) => {
    const DIMENSION_MAP = {
      1: {
        width: CANVAS_WIDTH,           // Full width for single flag
        flagHeight: FLAG_HEIGHT             // Full height for single flag
      },
      2: {
        width: CANVAS_WIDTH * 0.75,    // 75% width for dual flags (240px)
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      {activeTab === "size" ? (
        <>
          <h1 className="lg:text-lg text-md lg:font-bold font-semibold lg:mb-3 mb-1 text-gray-900">Shorts</h1>
          {/* Color — 2-row grid */}
          <div className="mb-4">
