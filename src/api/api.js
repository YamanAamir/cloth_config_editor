...
export const listSchools        = (params = {})  => api.post('/student/schools', params);
...
// // ── Admin / Class Rep: Order management ──────────────────────────────────────
...
// ── Payment ───────────────────────────────────────────────────────────────────
// First payment OR additional payment after edit (partial_paid flow)
export const createCheckoutSession = (data) => api.post('/payment/create-checkout-session', data);
// Alias used by some components
export const initiatePayment       = (data) => api.post('/payment/create-checkout-session', data);

// Per-product payment breakdown for an order
export const getOrderPaymentBreakdown = (orderId) => api.get(`/payment/breakdown/${orderId}`);

// Success-page fallback — ask backend to confirm payment directly with Stripe
// in case the webhook hasn't updated the order yet
export const verifyPaymentSession = (sessionId) => api.get(`/payment/verify-session/${sessionId}`);

// Price estimate before placing order
export const getOrderPricing = (data) => api.post('/payment/calculate-pricing', data);
